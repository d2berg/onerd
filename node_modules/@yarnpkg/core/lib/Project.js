...
class Project {
    ...
    async addWorkspace(workspaceCwd) {
        ...
        if (typeof dup !== `undefined`)
            throw new Error(`Duplicate workspace name ${structUtils.prettyIdent(this.configuration, workspace.locator)}: ${fslib_2.npath.fromPortablePath(workspaceCwd)} conflicts with ${fslib_2.npath.fromPortablePath(dup.cwd)}`);
    ...
    refreshWorkspaceDependencies() {
        for (const workspace of this.workspaces) {
            ...
            if (!pkg)
                throw new Error(`Assertion failed: Expected workspace ${structUtils.prettyWorkspace(this.configuration, workspace)} (${formatUtils.pretty(this.configuration, fslib_1.ppath.join(workspace.cwd, fslib_1.Filename.manifest), formatUtils.Type.PATH)}) to have been resolved. Run "yarn install" to update the lockfile`);
    ...
    async restoreInstallState({ restoreInstallersCustomData = true, restoreResolutions = true, restoreBuildState = true } = {}) {
        ...
        if (restoreBuildState)
            Object.assign(this, pick_1.default(installState, INSTALL_STATE_FIELDS.restoreBuildState));
...
function applyVirtualResolutionMutations({ project, allDescriptors, allResolutions, allPackages, accessibleLocators = new Set(), optionalBuilds = new Set(), volatileDescriptors = new Set(), peerRequirements = new Map(), report, tolerateMissingPackages = false, }) {
    ...
    const reportStackOverflow = () => {
        ...
        fslib_1.xfs.detachTemp(logDir);
        throw new Report_1.ReportError(MessageName_1.MessageName.STACK_OVERFLOW_RESOLUTION, `Encountered a stack overflow when resolving peer dependencies; cf ${fslib_2.npath.fromPortablePath(logFile)}`);
    };
