 ...
 */
export const getSpoolBinaryCommand = (packageNumber) => {
 ...
 */
export const getMessageBinaryCommand = (packageNumber) => {
 ...
 */
export const getSetClockCommand = (date) => {
    if (!(date instanceof Date)) {
        date = new Date(date);
    }
    let sc = new Uint8Array([47, 83, 67]); // /SC
    let clock = new Uint8Array([
        Number((date.getFullYear() + "").substring(2)),
        date.getMonth() + 1,
        date.getDate(),
        date.getHours(),
        date.getMinutes(),
