 ...
 */
export const int32ToBytes = (number) => {
 ...
 */
export const bytesToDate = (view) => {
    if (view.byteLength !== 6) {
        throw new Error(`Can't convert view ${view.byteLength} bytes to date, got ${view}`);
    }
    ...
    return new Date(year, view.getUint8(1) - 1, // Month (zero indexed in JavaScript)
 ...
 */
export const checkControlCode = (view, controlByte = 0) => {
    let sum = 0;
    for (let i = 0; i < view.byteLength; i++) {
        sum += view.getUint8(i);
    }
